...
function processRecurring(list, entryType) {
  ...
  list.forEach(rec => {
    ...
    // Si el día configurado todavía no llegó, el movimiento se genera para
    // que quede visible ("viene"), pero pending:true evita que pese en el
    // balance del mes hasta que la fecha de hoy lo alcance.
    const pending = day > now.getDate();
...
/* Revisa los movimientos autogenerados marcados pending:true y, si la
...
function reconcilePendingRecurring() {
  ...
  state.entries.forEach(e => {
    if (e.autoGenerated && e.pending && e.date <= today) {
      e.pending = false;
      changed = true;
    }
  });
