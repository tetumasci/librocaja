...
function processRecurring(list, entryType) {
  ...
  list.forEach(rec => {
    ...
    // Ingresos fijos: si el día configurado todavía no llegó, se genera
    // igual para que quede visible ("viene"), pero pending:true evita que
    // pese en el balance hasta que la fecha de hoy lo alcance (se resuelve
    // solo en reconcilePendingRecurring()).
    // Gastos fijos: siempre nacen pending:true, sin importar el día —
    // dejan de contar en el balance hasta que el usuario confirma el pago
    // a mano con el botón "pagar" (dayOfMonth queda como referencia visual).
    const pending = entryType === 'expense' ? true : (day > now.getDate());
...
/* Revisa los ingresos fijos autogenerados marcados pending:true y, si la
   fecha de hoy ya alcanzó o superó su fecha, los pasa a estado normal
   para que empiecen a sumar/restar del balance. Se corre en cada
   apertura de la app, antes del primer render.
   Solo aplica a ingresos: los gastos fijos (y, más adelante, las cuotas
   de compras en cuotas) usan confirmación manual con el botón "pagar" —
   nunca se auto-confirman por el solo paso de la fecha. */
function reconcilePendingRecurring() {
  const today = todayISO();
  let changed = false;
  state.entries.forEach(e => {
    if (e.autoGenerated && e.pending && e.type === 'income' && e.date <= today) {
      e.pending = false;
      changed = true;
    }
  });
...
/* Confirmación manual de pago: usada por el botón "pagar" del action
   sheet para gastos fijos con pending:true, y reusada por compras en
   cuotas (mismo patrón: generación mensual automática + confirmación
   manual, nunca automática por fecha). */
function confirmPendingPayment(entryId) {
