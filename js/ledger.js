...
/* Saldo total real: suma de todas las cuentas al día de hoy.
   A diferencia de month-balance, no depende de viewDate — siempre
   refleja el estado actual, sin importar qué mes se esté navegando. */
function renderTotalBalance() {
  const totalEl = document.getElementById('total-balance');
  if (!totalEl) return;
  const total = state.accounts.reduce((sum, acc) => sum + getAccountBalance(acc.id), 0);
...
function computeSuggestion() {
  ...
  const adjEntries = thisMonthEntries.filter(e => e.type === 'adjustment');
  if (adjEntries.length > 0) {
    const adjAbsTotal = adjEntries.reduce((s, e) => s + Math.abs(e.amount), 0);
    const expTotal = thisMonthEntries.filter(e => e.type === 'expense').reduce((s, e) => s + e.amount, 0);
    if (adjAbsTotal > 0 && (expTotal === 0 || adjAbsTotal / expTotal > 0.05)) {
      return `Este mes tuviste ${formatMoney(adjAbsTotal)} en ajustes de saldo sin identificar — puede que se te esté escapando algún gasto sin cargar.`;
    }
  }
  ...
  const adjNetSavings = thisMonthEntries.filter(e => e.type === 'adjustment').reduce((s, e) => s + e.amount, 0);
  if (income > 0) {
    const rate = ((income - expense + adjNetSavings) / income) * 100;
...
function renderEntryRow(entry) {
  ...
  const row = document.createElement('div');
  row.className = 'ledger-entry' + (entry.pending ? ' pending' : '');
  const subtitleParts = [];
  if (entry.autoGenerated) subtitleParts.push(entry.pending ? '↻ programado' : '↻');
  if (entry.subcategoryId && !isAdj) {
    const subcat = getSubcategoryById(entry.categoryId, entry.subcategoryId, entry.type);
    if (subcat) subtitleParts.push(escapeHtml(subcat.name));
  }
  if (entry.note && !isAdj) subtitleParts.push(escapeHtml(entry.note));
  if (isAdj) subtitleParts.push('diferencia no identificada');
...
function openEditModal(entry) {
  ...
  selectedSubcategoryId = entry.subcategoryId || null;
